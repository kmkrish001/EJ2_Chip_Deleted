    ...
    if (user && token) {
        ...
        rl.question('Git Lab UserName:', (userName) => {
            rl.question('Git Lab Password:', (pwd) => {
                rl.close();
                themeclone(userName, pwd, done, 'ej2-theme-studio', branchName);
            });
        });
    }
}
...
function themeclone(user, token, done, name, branchName) {
 ...
 //   token = token.replace('@', '%40');
    var tempRepo = 'https://' + user + ':' + token + '@gitlab.syncfusion.com/essential-studio/' + name + ' -b ' + branchName;
    shelljs.exec(
